...
3.  **选择音频设备**
    ...
    脚本执行成功后，打开你的系统“声音”设置。在“输入”设备列表中，选择名称中带有 **`echo cancellation`** 字样的虚拟麦克风设备。

    > 注意！此时扬声器设备将同样带有该字样，同时设定麦克风和扬声器为带有 **`echo cancellation`** 字样的设备，即可生效。
...
**Q1: 为什么使用独立的麦克风和扬声器没有效果？**

A: 请查看安装与配置步骤3，查看系统设置中，扬声器是否也设定为为带有 **`echo cancellation`** 字样的设备。

**Q2: 为什么使用笔记本自带的麦克风和扬声器没有效果？**
...
2.  **内置麦克风不支持AEC**
    ...
    在使用本项目脚本时选择了某些设备的内置麦克风，运行完毕后在系统设置里找不到带有 **`echo cancellation`** 字样的虚拟麦克风设备，说明内置麦克风不支持AEC，请使用独立的麦克风输入。
